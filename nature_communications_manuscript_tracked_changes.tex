...
Figure 1 presents the complete theoretical framework as a causal pipeline. Panel~(a) displays theoretical terrain PSDs in log-log coordinates for five fractal dimensions, confirming the power-law form $S_z(k) \propto k^{-\beta_t}$ with slopes governed by $\beta_t = 7-2D$. Panel~(b) shows the vehicle suspension transfer function $|H(\omega)|^2$, which acts as a bandpass filter peaking at the natural frequency $\omega_n$. Panel~(c) illustrates the two-parameter energy model $E \propto C_z^{0.94} \times \beta_t^{-0.09}$, showing that vibration energy scales nearly linearly with spectral amplitude $C_z$ while spectral slope provides a secondary modulation. Panel~(d) presents the fatigue scaling law $N_f \propto E^{-m/2}$ for different Basquin exponents $m$, completing the chain from vibration energy to fatigue-life proxy.
