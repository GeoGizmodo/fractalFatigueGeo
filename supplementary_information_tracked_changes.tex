...
{\large \textcolor{blue}{Spectral scaling of natural-terrain-induced fatigue}}
...
\textcolor{blue}{\textbf{Note:} All symbols used in this Supplementary Information are defined in the main manuscript ``Notation and definitions'' paragraph (Results section). Key definitions are repeated below where needed for readability. Symbols retain consistent meaning across both documents.}
...
The consistency across vehicles (CV = 2.7\% for MI values) mirrors the consistency observed in correlation coefficients ($r = -0.957$, $-0.968$, $-0.960$; CV = 0.2\%) and confirms that the $D \rightarrow \beta_a$ relationship is \textcolor{blue}{terrain-dominated within this narrow frequency range (0.89--0.98 Hz)}.
...
\textcolor{blue}{The low MI variability across the three narrow-frequency vehicles (CV = 2.7\%)} demonstrates that this information-theoretic relationship is intrinsic to the terrain-vehicle interaction and does not depend on specific suspension parameters within this frequency range. This supports the main conclusion that terrain fractal dimension serves as a \textcolor{blue}{terrain-dominated severity index, with natural frequency providing predictable modulation across broader vehicle ensembles}.
...
\item \textcolor{blue}{The terrain-dominated scaling remains valid regardless of this bias, as it affects all vehicles equally}
\end{enumerate}
...
\textcolor{blue}{The frequency dependence does not invalidate the terrain-dominated framework. Instead, it refines it:}

\begin{enumerate}
\item \textbf{Terrain still dominates}: \textcolor{blue}{Terrain geometry explains 93.5\% of variance; frequency modulates the remaining variation.}

\item \textbf{Predictable correction}: \textcolor{blue}{The relationship $\gamma(f_n)$ is linear and deterministic, enabling estimation of relative fatigue-risk trends for vehicles with known natural frequency within the modeled range.}

\item \textbf{Physically meaningful}: The dependence arises from fundamental spectral integration, not numerical artifacts.

\item \textbf{Practical utility}: \textcolor{blue}{For a vehicle fleet with known natural frequencies, relative fatigue-risk scaling can be estimated from terrain data using $\gamma = 0.36 f_n - 2.93$, though absolute fatigue-life prediction requires component-specific calibration of $K_\sigma$.}
\end{enumerate}
...
\section{\textcolor{blue}{Supplementary Note 10: Terrain-Dominated Scaling Validation}}

\subsection{Three-Vehicle Simulation Campaign}

\textcolor{blue}{To validate the robustness of the terrain-dominated scaling law,} we conducted a comprehensive simulation campaign across three vehicles with substantially different parameters:
...
Each vehicle was simulated over 500 terrain realizations (5 fractal dimensions $\times$ 100 realizations), yielding 1500 total simulations. All simulations used identical terrain generation parameters to isolate vehicle effects. The variance decomposition analysis shows that terrain fractal dimension explains \textcolor{blue}{93.5\%} of vibration energy variance through the amplitude-complexity coupling in our terrain generation algorithm, while vehicle type accounts for only \textcolor{blue}{1.9\%}, with the combined model explaining \textcolor{blue}{95.4\%}.

\subsection{\textcolor{blue}{Scaling Results}}
...
\subsection{\textcolor{blue}{Implications for Generality}}

\textcolor{blue}{The terrain-dominated scaling has several important implications:}
...
\item \textcolor{blue}{The framework enables cross-vehicle predictions within the narrow frequency range tested (0.89--0.98 Hz): once calibrated for one vehicle, the scaling can be transferred to other vehicles with similar natural frequency. For broader fleets spanning wider frequency ranges, a natural-frequency correction ($\gamma = 0.36f_n - 2.93$) is required.}

\item \textcolor{blue}{The low coefficient of variation (1.4\%) indicates the scaling law is robust to vehicle parameter variations within this narrow-frequency set (25-50\% variations in mass and stiffness at 0.89--0.98 Hz).}
...
\textcolor{blue}{\textbf{Energy--fractal-dimension relationship at constant amplitude:}}
\begin{equation}
\textcolor{blue}{\log_{10} E = 2.96 \cdot D - 5.49 \quad (R^2 = 0.834,\; r = 0.913,\; p < 10^{-176},\; n = 449)}
\end{equation}

\textcolor{blue}{Energy \emph{increases} with $D$ at constant amplitude (positive slope = $+2.96$), opposite to the coupled case (negative slope = $-3.11$). This sign reversal confirms that in the original simulations, the amplitude decrease with $D$ dominated over the spectral structure effect, masking the independent contribution of $\beta$.}
...
\textcolor{blue}{Together, LiRA-CD and TartanDrive provide complementary evidence: LiRA-CD demonstrates statistical significance through sample size on paved roads, while TartanDrive demonstrates large effect sizes on the intended off-road terrain domain. The combination provides complementary evidence at adjacent links in the chain, while still falling short of a single end-to-end validation dataset with all links measured simultaneously.}

\textcolor{blue}{\subsection{Dataset Description}}

\textcolor{blue}{The TartanDrive dataset\cite{Triest2022} was collected at Carnegie Mellon University's off-road test facility using a Yamaha Viking all-terrain vehicle (ATV) equipped with a Multisense S21 sensor suite. The vehicle traversed diverse natural terrain including forest trails, gravel paths, open fields, and areas with exposed roots and rocks. Key sensor specifications:}
...
\textcolor{blue}{\subsection{Analysis Methods}}

\textcolor{blue}{Vertical acceleration ($a_z(t)$) was extracted from the IMU linear\_acceleration.z channel at 200~Hz. The signal processing pipeline mirrors the simulation analysis:}
...
\textcolor{blue}{A total of 42 valid segments were extracted from 9 traversals.}

\textcolor{blue}{\subsection{Results}}
...
\textcolor{blue}{\textbf{Key findings:}}
{\color{blue}
\begin{itemize}
...
\item $\beta_a$ is speed-independent, consistent with the interpretation that spectral slope reflects terrain-related excitation rather than speed alone.
\end{itemize}
}

\textcolor{blue}{\subsection{Comparison with LiRA-CD Paved-Road Validation}}
...
\textcolor{blue}{The substantially stronger effect sizes on off-road terrain ($r = 0.512$--$0.880$) compared to paved roads ($r = 0.145$--$0.194$) confirm that the framework is most predictive for natural terrain where macro-topographic geometry dominates vehicle excitation. The LiRA-CD achieves statistical significance through sample size alone; TartanDrive achieves it through effect size on the intended terrain domain.}

\textcolor{blue}{\subsection{Limitations and Future Work}}

\textcolor{blue}{The current TartanDrive analysis has three limitations that future work could address:}
