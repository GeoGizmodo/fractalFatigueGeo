...
Together, three independent real-data analyses test adjacent links in the prediction chain: USGS LiDAR validates $D \rightarrow \beta_t$ on natural terrain geometry ($r = -0.62$, $p < 0.05$, 13~tiles); LiRA-CD validates $\beta_a \rightarrow E$ on real vehicle dynamics with large sample size ($r = +0.194$, $p < 10^{-18}$, 8,609~paved-road segments); and TartanDrive validates $\beta_a \rightarrow E$ and speed scaling on real off-road terrain with strong effect sizes ($r = 0.512$, $p < 10^{-3}$, 42~segments). These do not constitute a single end-to-end validation, but their combined evidence---from independent methods, vehicles, and geographic settings---supports the existence of the predicted relationships at each step, with effect sizes increasing as the measurement domain approaches the framework's intended natural-terrain application.

\textbf{Domain boundary: the Copenhagen falsification test.} A critical prediction of the framework is that it should \emph{fail} on paved roads, where centimeter-scale pavement texture---not meter-scale macro-topography---governs vehicle excitation. We tested this prediction directly using Copernicus GLO-30 DEM (30 m resolution, $n=972$ segments) and Danish DHM (1 m resolution, $n=128$ segments) co-located with LiRA-CD vehicle routes in Copenhagen. The result confirms the prediction: increasing DEM resolution from 30~m to 1~m does not strengthen the terrain--vibration relationship (correlation decreases from $r = +0.102$ to $r = +0.037$, both wrong-sign and statistically indistinguishable from zero at higher resolution). The Copenhagen terrain is topographically flat (elevation range $-16.5$ to $+100.5$~m, $D_1$ IQR spanning only 0.13--0.18 units), confirming that the relevant roughness operates at spatial scales below DEM resolution. This clean falsification establishes a precise domain boundary: the framework applies where DEM-resolved macro-topography dominates excitation (natural terrain, off-road operations) and does not apply where pavement micro-texture governs vibration (maintained roads). The USGS mountain terrain comparison ($r = -0.583$, $p = 0.036$) provides preliminary support in the intended domain, and the TartanDrive off-road analysis ($r = 0.512$, $p < 10^{-3}$) confirms substantially stronger effects on natural terrain. Figure 6 presents this domain boundary visually.
...
\textbf{a}, Terrain power spectral densities for five fractal dimensions ($D = 2.1$--$2.5$), demonstrating the power-law form $S_z(k) \propto k^{-\beta_t}$ with slopes governed by $\beta_t = 7-2D$. Smooth terrain (low $D$) has steep spectral roll-off; rough terrain (high $D$) distributes energy more uniformly across wavenumbers.
\textbf{b}, Vehicle suspension transfer function $|H(\omega)|^2$ showing bandpass filtering with peak at natural frequency $\omega_n$. This step converts terrain PSD into vehicle acceleration PSD.
\textbf{c}, Two-parameter energy model $E \propto C_z^{0.94} \times \beta_t^{-0.09}$ showing vibration energy as a function of spectral amplitude $C_z$ for three spectral slopes. Energy scales nearly linearly with amplitude; slope provides secondary modulation.
\textbf{d}, Fatigue scaling law $N_f \propto E^{-m/2}$ for Basquin exponents $m = 3, 4, 5$, illustrating the strong nonlinear sensitivity of fatigue life to vibration energy magnitude.
