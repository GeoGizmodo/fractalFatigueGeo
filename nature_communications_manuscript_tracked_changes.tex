% Nature Communications Manuscript â€” REVISED (Comments Addressed)
% Spectral scaling of natural-terrain-induced fatigue
% TRACKED CHANGES VERSION: All revised text shown in blue
...
% TITLE PAGE
\begin{center}
{\Large\bfseries \textcolor{blue}{Spectral scaling of natural-terrain-induced fatigue}}
...
% ABSTRACT
\begin{abstract}
Terrain-induced mechanical wear remains unpredictable despite decades of vehicle dynamics research. Here we establish that terrain spectral structure, characterized by a two-parameter power spectral density framework (amplitude $C_z$ and slope $\beta$), governs vehicle vibration severity and fatigue accumulation. Monte Carlo simulations across 1500 terrain realizations using 2-DOF quarter-car models for three vehicle classes demonstrate that terrain geometry explains \textcolor{blue}{93.5\% of vibration energy variance while vehicle configuration explains only 1.9\% within the three-vehicle narrow-frequency comparison (0.89--0.98 Hz); broader ensemble simulations across 100 vehicles reveal systematic natural-frequency modulation of the scaling exponent ($\gamma = 0.36f_n - 2.93$, $R^2 = 0.879$),} with fractal dimension providing the geometric basis for spectral slope. Critically, terrain severity depends on both spectral amplitude $C_z$ and slope $\beta_a$, as revealed by the two-parameter model $\mathbf{E \propto C_z^{0.94} \times \beta_a^{-0.09}}$ (R$^2$ = 0.96), where amplitude dominates energy magnitude while fractal dimension determines spectral structure through $\beta_t = 7-2D$. The vehicle acceleration spectral exponent $\beta_a$ correlates strongly with terrain fractal dimension ($r = -0.956$, 95\% CI: $[-0.959, -0.952]$, $n=1500$), with empirical slope $-1.59$ due to suspension filtering. Extended validation across 100 vehicles (18,000 simulations) reveals that the fatigue scaling exponent $\gamma = -2.34 \pm 0.19$ (CV = 8.1\%) is independent of vehicle mass (R$^2$ = 0.000) and damping (R$^2$ = 0.047), but exhibits systematic frequency dependence arising from spectral filtering. Cumulative fatigue damage varies non-monotonically with fractal dimension due to amplitude-complexity coupling in synthetic terrain generation (an artifact of the Diamond-Square algorithm), spanning a 204-fold range. Preliminary comparison with USGS 3DEP LiDAR-derived digital elevation models (13 regions at 10 m resolution; expansion to 25 class-balanced regions in progress) spanning diverse geomorphological roughness classes \textcolor{blue}{provides empirical evidence for a negative $\beta_t$--$D$ relationship consistent in sign with the theoretical scaling.} Weighted regression yields $r = -0.62$ ($p < 0.05$), with Spearman $\rho = -0.53$ ($p = 0.061$) providing supporting evidence of robustness. Analysis of 8,609 road segments from the LiRA-CD in-vehicle sensor dataset shows that $\beta_a$ and vibration energy correlate with measured IRI ($r = +0.194$ and $r = +0.145$, both $p < 10^{-18}$). These results establish a two-parameter spectral framework $(C_z, \beta_a)$ \textcolor{blue}{for estimating vibration severity and relative fatigue-risk scaling under natural-terrain excitation}, where fractal dimension provides the geometric basis for spectral slope while amplitude measurements capture terrain severity. \textcolor{blue}{This framework enables quantitative estimation of vibration severity and relative fatigue-risk trends for natural terrain where digital elevation models are available; it is not applicable to engineered paved surfaces.}
\end{abstract}
...
\subsection*{\textcolor{blue}{Terrain-dominated scaling and multi-vehicle validation}}
...
\hline
Terrain $D$ only & \textcolor{blue}{0.935} & \textcolor{blue}{93.5\%} \\
Vehicle type only & \textcolor{blue}{0.019} & \textcolor{blue}{1.9\%} \\
$D$ + Vehicle (combined) & \textcolor{blue}{0.954} & \textcolor{blue}{95.4\%} \\
\hline
\end{tabular}
\label{tab:variance}
\end{table}

\textcolor{blue}{Terrain fractal dimension explains 93.5\% of vibration energy variance across all vehicles through the amplitude-complexity coupling in our terrain generation algorithm, while vehicle type accounts for only 1.9\% within this narrow-frequency comparison. This provides strong quantitative evidence that $\beta$ (determined by $D$ through $\beta = 7-2D$) serves as a terrain-dominated severity index when combined with amplitude measurement, with the caveat that natural frequency modulates the scaling exponent predictably across a broader vehicle ensemble (Fig.~\ref{fig:ensemble}). The combined model explains 95.4\% of variance, leaving only 4.6\% unexplained (attributable to stochastic variation in terrain generation).} This variance decomposition demonstrates that terrain geometry, characterized by the two-parameter framework $(C_z, \beta)$, predicts vibration energy across different vehicle configurations.

\textcolor{blue}{These results have several practical implications.} First, the scaling exponent depends primarily on terrain geometry (through the amplitude-complexity coupling), with systematic modulation by vehicle resonance frequency. Second, different vehicles experience different absolute energy levels but follow predictable scaling with fractal dimension. Third, the framework enables cross-vehicle predictions: once the scaling is calibrated for one vehicle class, it can be transferred to other vehicles by adjusting for frequency-dependent effects. \textcolor{blue}{Fourth, the low coefficient of variation (1.4\%) and high variance explained by terrain (93.5\%) confirm that the scaling is terrain-dominated within the narrow frequency range tested; the broader ensemble (Fig.~\ref{fig:ensemble}) shows that across vehicles with wider natural-frequency spread, response is predictably modulated by natural frequency rather than being truly vehicle-independent.}
...
To test the complete prediction chain ($D \rightarrow \beta \rightarrow E$) using real vehicle dynamics, we analyzed the LiRA-CD dataset\cite{Skar2023}, an open-source collection of GPS-tagged in-vehicle sensor signals from Renault Zo\'{e} electric vehicles (GreenMobility fleet, Copenhagen) measured over 230 km of urban and highway roads. This dataset provides 8,609 road segments with synchronized vehicle acceleration and terrain roughness measurements. Reference road condition parameters, including International Roughness Index (IRI) at 10 m spatial resolution, were obtained from parallel P79 Profilometer measurements by the Danish Road Directorate.
...
Together, the LiDAR terrain comparison and LiRA-CD vehicle analysis trace the full prediction chain empirically using independent datasets. The LiDAR analysis confirmed $D \rightarrow \beta$ on real terrain geometry; the LiRA-CD analysis confirmed $\beta \rightarrow E$ on real vehicle dynamics ($n = 8{,}609$ segments). The two validations use independent measurement methods and geographic settings, making their combined confirmation robust. While the LiDAR comparison is preliminary (13 tiles), the LiRA-CD analysis is comprehensive (8,609 segments), providing strong evidence that the framework applies to real-world vehicle-terrain interactions.
...
\textcolor{blue}{To validate the framework on real off-road terrain---the domain for which it is specifically intended---we analyzed the TartanDrive autonomous driving dataset\cite{Triest2022}, which contains synchronized IMU accelerometer data (200~Hz) and GPS-tagged odometry from a Yamaha Viking all-terrain vehicle (ATV) traversing diverse natural terrain (forest trails, gravel paths, open fields) at Carnegie Mellon University's off-road test facility. Vehicle speeds ranged from 0.01 to 5.89~m/s (mean 1.42~m/s), spanning the low-speed off-road regime relevant to military and autonomous applications.}
...
\textbf{Diamond step:} For each square in the grid, the center point is set to the average of the four corners plus a random offset drawn from $\mathcal{N}(0, \sigma_k^2)$, where \textcolor{blue}{$\sigma_k = \sigma_0 \cdot r_{\text{DS}}^{kH}$ with $r_{\text{DS}} = 0.5$ (the Diamond-Square variance reduction factor)} and $k$ is the recursion level.
...
\textbf{Recursion:} The process repeats with progressively smaller squares until all grid points are assigned. The variance reduction factor \textcolor{blue}{$r_{\text{DS}}^{kH}$} ensures the correct Hurst exponent scaling.
...
\textbf{Model parameters:} The HMMWV has a total mass of 3402 kg distributed across four wheels. The 2-DOF quarter-car model includes: sprung mass $m_s = 850.5$ kg (one-quarter of vehicle body mass), unsprung mass $m_u = 100$ kg (wheel and axle assembly), suspension spring stiffness $k_s = 36,300$ N/m, suspension damping coefficient \textcolor{blue}{$c_s = 2\zeta\sqrt{k_s m_s} = 3,335$ N$\cdot$s/m (computed from target $\zeta = 0.30$)}, and tire stiffness $k_t = 200,000$ N/m. These parameters yield body natural frequency $\omega_n = \sqrt{k_s/m_s} = 6.53$ rad/s (1.04 Hz) and damping ratio $\zeta = 0.30$, representing a lightly damped system typical of military vehicle suspensions. All simulations use $\zeta = 0.30$ for consistency.
...
\textcolor{blue}{\textbf{b}, Two-parameter model $E \propto C_z^{0.94} \times \beta_a^{-0.09}$ achieves $R^2 = 0.96$; $C_z$ dominates energy prediction in this coupled dataset while $\beta$ provides the geometric link to fractal dimension.} 
...
\textcolor{blue}{Extended validation across 100 vehicles (18,000 simulations) spanning 14$\times$ mass range (172--2482 kg) and 3.1$\times$ frequency range (0.80--2.49 Hz).} 
