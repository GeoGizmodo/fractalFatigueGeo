...
In the original 1500 simulations, spectral amplitude $C_z$ and spectral slope $\beta$ are strongly correlated ($r = -0.962$) due to the amplitude-complexity coupling inherent in the Diamond-Square terrain generation algorithm. Partial correlation analysis (Supplementary Figure~3) shows that after controlling for $C_z$, $\beta$ explains essentially no residual energy variance ($R^2 = 0.005$). This raises the question of whether spectral structure independently affects vibration energy, or whether $C_z$ alone is a sufficient predictor.

To resolve this, we conducted a decoupling experiment in which terrain amplitude is held exactly constant while spectral structure (fractal dimension $D$, and therefore $\beta = 7-2D$) is varied independently.
