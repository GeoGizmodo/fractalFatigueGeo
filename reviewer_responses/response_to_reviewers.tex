...
No new simulations are required to address this comment. The response involves honest reframing of existing evidence and identification of validation gaps.

\revloc{Abstract (final sentences, domain statement); Discussion (validation limitations paragraph, revised); Data Sources table (added for transparency); throughout manuscript where prediction claims were moderated.}
...
\noindent Critically, the TartanDrive off-road effect sizes ($r = 0.512$--$0.880$) are \emph{substantially} stronger than LiRA-CD paved-road results ($r = 0.145$--$0.194$). This confirms the framework's design intent and directly addresses the reviewer's concern: the framework is most predictive precisely for the natural-terrain domain where it is intended to apply.
