...
The structural parallel is close: ISO~8608 models road PSD as $G_d(n) = G_d(n_0)(n/n_0)^{-w}$, where $G_d(n_0)$ is a spectral amplitude and $w$ is a waviness exponent. Our $(C_z, \beta)$ framework uses the same two-parameter PSD logic---$C_z$ maps directly onto $G_d(n_0)$, and $\beta$ maps onto $w$. The difference is not the mathematical form; it is what you can do with each approach before a vehicle has ever crossed the terrain.

The three concrete advantages are as follows. First, ISO~8608 requires a profilometer measurement to determine $w$ for a given surface; the standard treats $w \approx 2$ as a fixed empirical constant when no measurement is available. Our framework derives $\beta$ from terrain geometry through $\beta = 7-2D$, so the spectral slope can be predicted from a digital elevation model alone, without any in-situ instrumentation. Second, ISO~8608 road classes (A through H) were calibrated on paved and graded surfaces. They do not extend to natural terrain---mountains, desert washes, off-road military routes, or planetary surfaces---where no road classification exists. The fractal framework is specifically validated in that domain (13 USGS 3DEP regions spanning plains to high-relief mountain terrain). Third, standard ISO~8608 practice fixes $w$ and classifies roads by amplitude alone. Explicitly fitting both parameters improves prediction of vibration energy from $R^2 = 0.91$ (amplitude-only baseline) to $R^2 = 0.96$ in our simulations. That 5-point gain in $R^2$ is modest in absolute terms, but represents a measurable reduction in residual variance; because fatigue life scales as $N_f \propto E^{-m/2}$ with exponents $m$ typically between 3 and 10, even small errors in $E$ compound into large errors in predicted component life.
...
\bibitem{Braut2023}
Braut, S., \v{Z}iguli\'{c}, R., Butkovi\'{c}, A., \v{C}avka, D. \& Bo\v{z}i\'{c}, \v{Z}. Fractal dimension as a descriptor of vehicle suspension system degradation for predictive maintenance. \textit{Eng. Fail. Anal.} \textbf{145}, 107014 (2023).
