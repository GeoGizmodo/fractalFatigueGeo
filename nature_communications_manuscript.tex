...
% Universal spectral scaling of terrain-induced fatigue
...
% TITLE PAGE
\begin{center}
{\Large\bfseries Universal spectral scaling of terrain-induced fatigue}
...
% ABSTRACT
\begin{abstract}
Terrain-induced mechanical wear remains unpredictable despite decades of vehicle dynamics research. Here we establish that terrain spectral structure, characterized by a two-parameter power spectral density framework (amplitude $C_z$ and slope $\beta$), governs vehicle vibration severity and fatigue accumulation. Monte Carlo simulations across 1500 terrain realizations using 2-DOF quarter-car models for three vehicle classes demonstrate that terrain geometry explains 95.1\% of vibration energy variance while vehicle configuration explains only 2.0\%, with fractal dimension providing the geometric basis for spectral slope. Critically, terrain severity depends on both spectral amplitude $C_z$ and slope $\beta_a$, as revealed by the two-parameter model $\mathbf{E \propto C_z^{0.94} \times \beta_a^{-0.09}}$ (R$^2$ = 0.96), where amplitude dominates energy magnitude while fractal dimension determines spectral structure through $\beta_t = 7-2D$. The vehicle acceleration spectral exponent $\beta_a$ correlates strongly with terrain fractal dimension ($r = -0.956$, 95\% CI: $[-0.959, -0.952]$, $n=1500$), with empirical slope $-1.59$ due to suspension filtering. Extended validation across 100 vehicles (18,000 simulations) reveals that the fatigue scaling exponent $\gamma = -2.34 \pm 0.19$ (CV = 8.1\%) is independent of vehicle mass (R$^2$ = 0.000) and damping (R$^2$ = 0.047), but exhibits systematic frequency dependence ($\gamma = 0.36 f_n - 2.93$, R$^2$ = 0.879) arising from spectral filtering: vehicles with higher resonance frequencies sample shorter terrain wavelengths where power-law spectra have lower amplitude. Cumulative fatigue damage varies non-monotonically with fractal dimension due to amplitude-complexity coupling in synthetic terrain generation (an artifact of the Diamond-Square algorithm), spanning a 204-fold range. Preliminary comparison with USGS 3DEP LiDAR-derived digital elevation models (13 regions at 10 m resolution; expansion to 25 class-balanced regions in progress) spanning diverse geomorphological roughness classes provides empirical evidence consistent with the theoretical $\beta_t$--$D$ scaling. Weighted regression yields $r = -0.62$ ($p < 0.05$), with Spearman $\rho = -0.53$ confirming robustness. Analysis of 8,609 road segments from the LiRA-CD in-vehicle sensor dataset shows that $\beta_a$ and vibration energy correlate with measured IRI ($r = +0.194$ and $r = +0.145$, both $p < 10^{-18}$). These results establish a two-parameter spectral framework $(C_z, \beta_a)$ for predicting terrain-induced mechanical fatigue, where fractal dimension provides the geometric basis for spectral slope while amplitude measurements capture terrain severity. This framework enables quantitative prediction of mechanical wear across diverse terrain conditions.
\end{abstract}
...
The mean exponent is $\gamma = -3.105 \pm 0.044$ with coefficient of variation CV = 1.4\%, demonstrating remarkable consistency across vehicles with 25-50\% variations in mass and stiffness. Combined scaling across all 1500 simulations yields $E \propto (D-2)^{-3.106}$ with $R^2 = 0.915$ and $p$-value less than $10^{-300}$, confirming the relationship reflects genuine physical scaling.
